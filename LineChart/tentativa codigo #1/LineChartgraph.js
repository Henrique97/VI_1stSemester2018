//ajustado para D3 v5
//ajustado de data so para ano



function createLineChart(margin, width, height, url, div, fields) {
    ...
    var formattedDateInterval = function(dateInterval) {
        return dateInterval[0].format('YYYY') + ' to ' + dateInterval[1].format('YYYY');
    };
    ...
    function loadFromDB(data_url) {
        d3.csv(data_url, function(error, data_points) {
            data_points = d3.csvParse(data_points)
            //data_points = d3.parse(data_points)
            dataset = data_points
              ...
              .enter().append("circle")
                ...
                .on("mouseover", function(d) {
                    div.transition()
                        .duration(200)
                        .style("opacity", .9);
                    div .html((moment(d.date).utc().format('YYYY')) + "<hr class='hrtooltip'> Total: "  + d[current_field] )
...
/*
    chart.updateFromDB = function (data_url) {
        d3.csv(data_url, function(error, data_points) {
            data_points = CSV.parse(data_points);
            dataset = data_points;
            dateInterval = d3.extent(data_points, function(d) { return moment(d.date).utc(); });
            xScale.domain(dateInterval);
            yScale.domain( [0, d3.max(data_points, function(d) { return d[current_field]; }) + 1] );
